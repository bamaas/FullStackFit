...
class Bodyweight(object):
    def insert(self, weight, table=TABLE_NAME):
        DatabaseHandler.execute_sql_query("INSERT INTO {} (datetime, weight) VALUES (current_timestamp, {})".format(table, weight))

    def get_all(self, table=TABLE_NAME):
        # TODO fix the select * FROM to select weight, datetime
        records = DatabaseHandler.execute_sql_query("SELECT * FROM {}".format(table))
