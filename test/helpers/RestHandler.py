...
class RestHandler(object):
    def sent_form_urlencoded_request(self, url, data, expected_status=200):
        ...
        ''' Posts a challenge (json) directly to the backend based on the json_resource argument. A custom title can be given.
            If None is given a title will be automatically generated by using the keyword 'generate challenge title'
            This function returns the response body as a dict object '''
        if title == None:
            title = BuiltIn().run_keyword("Generate Challenge Title")
        filepath = os.path.abspath(os.getcwd() + '/helpers/resources/' + json_resource)
        with open(filepath, 'r') as f:
            import json
            json_file = json.load(f)
            # Replace title if given
            if title is not None:
                if not isinstance(title, str):
                    raise TypeError("Title argument only accepts Strings")
                json_file['title'] = title
                data = json.dumps(json_file)
            else:
                data = f.read()
            data_object = json.loads(data)     # To python object
            data_print = {'data': (data_object)}    # Ugly construction but need it for pretty printing.
            data = {'data': (data)}     # We need to encode it as multipart/form-data. So we put the data in a tuple inside a dict
            header = {'kvkmasterid':kvkmasterid}     # This is the user/role
            print("--------- REQUEST ---------")
            print("Header: \n" + json.dumps(header, indent=4) + "\n")
            print("Body: \n", json.dumps(data_print, indent=4))
            r = requests.post('{}/challenges'.format(self.BE_URL), data=data, headers=header, )
            print("\n--------- RESPONSE --------- \n")
            print("Status code: " + str(r.status_code) + "\n")
            print("Header: \n" + json.dumps(dict(r.headers), indent=4) + "\n")
