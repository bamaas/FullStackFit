...
class RestHandler(object):
    def sent_form_urlencoded_request(self, url, data, expected_status=200):
        import json
        print("--------- REQUEST ---------")
        ...
        BuiltIn().set_test_variable("$OUTPUT", response_body)
        print("Body: \n" + json.dumps(response_body, indent=4))
        if r.status_code != expected_status:
            BuiltIn().fail(response_body)
        json_data = json.loads(r.text)
        return json_data

    def pretty_print_dictionary(self, data):
